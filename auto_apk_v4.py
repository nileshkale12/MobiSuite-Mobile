...
# Modern UI engine validation wrapper
try:
...
# Check missing network protocols dynamically
try:
    import paramiko
    from scp import SCPClient
    print("[+] Paramiko and SCP modules imported successfully.")
except ImportError:
    print("[!] Warning: Paramiko or SCP missing. Real-time installation options enabled.")
    paramiko = None
    SCPClient = None
...
class NKCyberSuiteMobile(ctk.CTk):
    def __init__(self):
        ...
        self.title("NK-CyberSuite Mobile v4")
        ...
        # Map Local Files System Binaries (5 Tools)
        self.bin_names = ["adb.exe", "apktool.jar", "APKEditor.jar", "zipalign.exe", "apksigner.jar"]
        self.bin_status_labels = {}

        # Map Python-m pip Virtual Environment Packages (UPDATED: Expanded to 5 Tools)
        ...
        logo_main = ctk.CTkLabel(self.sidebar, text="💎 NK-CYBERSUITE", font=ctk.CTkFont(size=18, weight="bold"), text_color="#00E5FF")
        ...
        logo_sub.pack(padx=20, pady=(2, 35))
        ...
        self.btn_nav_android.pack(padx=15, pady=6, fill="x")
        ...
        self.btn_nav_ios.pack(padx=15, pady=6, fill="x")
        ...
        self.btn_nav_settings.pack(padx=15, pady=6, fill="x")
        
        version_lbl = ctk.CTkLabel(self.sidebar, text="Version 4.0.0 (Production)", font=ctk.CTkFont(size=10), text_color="gray")
        version_lbl.pack(side="bottom", pady=15)

        # ---------------------------------------------------------------------
        # RIGHT MAIN PANEL DISPLAY SURFACES
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # LOWER RECONNAISSANCE CONSOLE LOGGER PANEL
        # ---------------------------------------------------------------------
        self.console_frame = ctk.CTkFrame(self.main_deck, fg_color="#141416", corner_radius=6, border_color="#222226", border_width=1)
        self.console_frame.pack(side="bottom", fill="x", padx=20, pady=20)
        
        console_lbl = ctk.CTkLabel(self.console_frame, text="LIVE ASSESSMENT STREAM TELEMETRY", font=ctk.CTkFont(size=10, weight="bold"), text_color="#7A7A80")
        console_lbl.pack(anchor="w", padx=15, pady=5)
        
        self.console = ctk.CTkTextbox(self.console_frame, height=140, fg_color="#09090B", text_color="#00E676", font=ctk.CTkFont(family="Consolas", size=12))
        self.console.pack(fill="both", expand=True, padx=10, pady=(0, 10))

        # Launch Dependency Scanner
        self.log("[*] NK-CyberSuite Engine online. Initializing environmental audits...")
    ...
    def switch_deck_context(self, target_deck):
        ...
        if target_deck == "android":
            ...
            self.view_android.pack(fill="both", expand=True)
        elif target_deck == "ios":
            self.btn_nav_ios.configure(fg_color="#2A2B36", text_color="#FFFFFF")
            self.view_ios.pack(fill="both", expand=True)
    ...
    # -------------------------------------------------------------------------
    # SETTINGS DECK WINDOW (Dynamically handles 5 System Binaries & 5 Pip Packages)
    # -------------------------------------------------------------------------
    def generate_settings_deck_ui(self):
        ...
        # Sub-Panel A: Local Binary Status Manager
        bin_card = ctk.CTkFrame(self.view_settings, fg_color="#16161A", corner_radius=6)
        bin_card.pack(fill="x", padx=20, pady=5)
        ...
        lbl_mon.pack(anchor="w", padx=20, pady=(12, 8))

        for bin_file in self.bin_names:
            row = ctk.CTkFrame(bin_card, fg_color="#1E1E24", height=36, corner_radius=4)
            row.pack(fill="x", padx=15, pady=2)
            row.pack_propagate(False)

            ctk.CTkLabel(row, text=f"  🛠️  {bin_file}", font=ctk.CTkFont(size=12)).pack(side="left", padx=5)
        ...
        # Sub-Panel B: Python-m Pip Module Status Manager (UPDATED to 5 items)
        pip_card = ctk.CTkFrame(self.view_settings, fg_color="#16161A", corner_radius=6)
        pip_card.pack(fill="x", padx=20, pady=5)
        ...
        lbl_pip.pack(anchor="w", padx=20, pady=(12, 8))

        for pkg in self.pip_packages:
            row = ctk.CTkFrame(pip_card, fg_color="#1E1E24", height=36, corner_radius=4)
            row.pack(fill="x", padx=15, pady=2)
            row.pack_propagate(False)
    ...
    def check_local_tools_inventory(self):
        ...
        # 1. Audit local drive binaries
        for bin_file in self.bin_names:
            if os.path.exists(os.path.join(tools_dir, bin_file)):
        ...
        # 2. Audit active environment site-packages
        for pkg in self.pip_packages:
    ...
    def generate_android_deck_ui(self):
        ...
        lbl_0a.pack(anchor="w", padx=15, pady=8)
        self.btn_scan_adb = ctk.CTkButton(card_0a, text="Scan USB Apps", fg_color="#0288D1", hover_color="#039BE5", command=self.start_android_package_fetch)
        self.btn_scan_adb.pack(side="left", padx=15, pady=(0, 15))
        self.cbo_android_apps = ctk.CTkComboBox(card_0a, values=["Click Scan to look up application lists..."], width=300)
        self.cbo_android_apps.pack(side="left", padx=5, pady=(0, 15))
        self.btn_pull_adb = ctk.CTkButton(card_0a, text="Pull Targets Folder", fg_color="#2E7D32", hover_color="#388E3C", command=self.start_android_apk_pull)
        self.btn_pull_adb.pack(side="right", padx=15, pady=(0, 15))

        # Step 0b Card: APKEditor Merger
        card_0b = ctk.CTkFrame(self.view_android, fg_color="#16161A", corner_radius=6)
        card_0b.pack(fill="x", padx=20, pady=6)
        ...
        lbl_0b.pack(anchor="w", padx=15, pady=8)
        btn_sel_merge = ctk.CTkButton(card_0b, text="Select Chunks Folder", fg_color="#37474F", hover_color="#455A64", command=self.browse_merge_folder)
        btn_sel_merge.pack(side="left", padx=15, pady=(0, 15))
        self.lbl_merge_dir = ctk.CTkLabel(card_0b, text="No Split App Folder Selected", text_color="gray", width=280, anchor="w")
        self.lbl_merge_dir.pack(side="left", padx=5, pady=(0, 15))
        self.btn_run_merge = ctk.CTkButton(card_0b, text="Merge via APKEditor", fg_color="#E65100", hover_color="#F57C00", command=self.start_apk_merger)
        self.btn_run_merge.pack(side="right", padx=15, pady=(0, 15))

        # Step 1 Card: Decompile Engine
        card_1 = ctk.CTkFrame(self.view_android, fg_color="#16161A", corner_radius=6)
        card_1.pack(fill="x", padx=20, pady=6)
        lbl_1 = ctk.CTkLabel(card_1, text="Step 1: Reverse Engineering Assembly Pipeline (Apktool)", font=ctk.CTkFont(size=12, weight="bold"))
        lbl_1.pack(anchor="w", padx=15, pady=8)
        btn_br_apk = ctk.CTkButton(card_1, text="Browse Target APK", command=self.browse_apk)
        btn_br_apk.pack(side="left", padx=15, pady=(0, 15))
        self.lbl_apk = ctk.CTkLabel(card_1, text="No active APK loaded as tracking target.", text_color="#A0A0A5", width=340, anchor="w")
        self.lbl_apk.pack(side="left", padx=5, pady=(0, 15))
        self.btn_decompile = ctk.CTkButton(card_1, text="Decompile", fg_color="#5E35B1", hover_color="#6F35B1", state="disabled", command=self.start_decompile)
        self.btn_decompile.pack(side="right", padx=15, pady=(0, 15))

        # Step 2 Card: Rebuild Engine
        card_2 = ctk.CTkFrame(self.view_android, fg_color="#16161A", corner_radius=6)
        card_2.pack(fill="x", padx=20, pady=6)
        lbl_2 = ctk.CTkLabel(card_2, text="Step 2: Package Compilation Reassembly & Jar Signer", font=ctk.CTkFont(size=12, weight="bold"))
        lbl_2.pack(anchor="w", padx=15, pady=8)
        btn_br_dir = ctk.CTkButton(card_2, text="Select Modded Folder", command=self.browse_folder)
        btn_br_dir.pack(side="left", padx=15, pady=(0, 15))
        self.lbl_dir = ctk.CTkLabel(card_2, text="No modified directory path selected.", text_color="#A0A0A5", width=340, anchor="w")
        self.lbl_dir.pack(side="left", padx=5, pady=(0, 15))
        self.btn_rebuild = ctk.CTkButton(card_2, text="Rebuild & Sign", fg_color="#1565C0", hover_color="#1E88E5", state="disabled", command=self.start_rebuild)
        self.btn_rebuild.pack(side="right", padx=15, pady=(0, 15))

        # Step 3 Card: Standalone Signer
        card_3 = ctk.CTkFrame(self.view_android, fg_color="#16161A", corner_radius=6)
        card_3.pack(fill="x", padx=20, pady=6)
        ...
        lbl_3.pack(anchor="w", padx=15, pady=8)
        btn_br_sign = ctk.CTkButton(card_3, text="Browse Manual APK", command=self.browse_unsigned_apk)
        btn_br_sign.pack(side="left", padx=15, pady=(0, 15))
        self.lbl_sign_apk = ctk.CTkLabel(card_3, text="No manual unsigned APK binary staged.", text_color="#A0A0A5", width=340, anchor="w")
        self.lbl_sign_apk.pack(side="left", padx=5, pady=(0, 15))
        self.btn_sign_only = ctk.CTkButton(card_3, text="Zipalign & Sign", fg_color="#C2185B", hover_color="#D81B60", state="disabled", command=self.start_sign_only)
        self.btn_sign_only.pack(side="right", padx=15, pady=(0, 15))
    ...
    def generate_ios_deck_ui(self):
        ...
        card_ssh.pack(fill="x", padx=20, pady=6)
        ...
        lbl_ssh.pack(anchor="w", padx=15, pady=8)
        
        ctk.CTkLabel(card_ssh, text="IP:").pack(side="left", padx=(15, 2), pady=(0, 15))
        self.ent_ios_ip = ctk.CTkEntry(card_ssh, width=130); self.ent_ios_ip.insert(0, "192.168.137.73"); self.ent_ios_ip.pack(side="left", pady=(0, 15))
        ctk.CTkLabel(card_ssh, text="User:").pack(side="left", padx=(10, 2), pady=(0, 15))
        self.ent_ios_user = ctk.CTkEntry(card_ssh, width=65); self.ent_ios_user.insert(0, "root"); self.ent_ios_user.pack(side="left", pady=(0, 15))
        ctk.CTkLabel(card_ssh, text="Pass:").pack(side="left", padx=(10, 2), pady=(0, 15))
        self.ent_ios_pass = ctk.CTkEntry(card_ssh, width=80, show="*"); self.ent_ios_pass.insert(0, "alpine"); self.ent_ios_pass.pack(side="left", pady=(0, 15))

        self.btn_fetch_apps = ctk.CTkButton(card_ssh, text="Scan App Bundles", fg_color="#6A1B9A", hover_color="#7B1FA2", width=120, command=self.start_ios_app_fetch)
        self.btn_fetch_apps.pack(side="right", padx=15, pady=(0, 15))

        card_sel = ctk.CTkFrame(self.view_ios, fg_color="#16161A", corner_radius=6)
        card_sel.pack(fill="x", padx=20, pady=6)
        lbl_sel = ctk.CTkLabel(card_sel, text="Step 2: Track Target Decrypted Bundle", font=ctk.CTkFont(size=12, weight="bold"))
        lbl_sel.pack(anchor="w", padx=15, pady=8)
        self.cbo_ios_apps = ctk.CTkComboBox(card_sel, values=["Run Remote Scan tracking logic first..."], width=670)
        self.cbo_ios_apps.pack(padx=15, pady=(0, 15), fill="x")

        card_pack = ctk.CTkFrame(self.view_ios, fg_color="#16161A", corner_radius=6)
        card_pack.pack(fill="x", padx=20, pady=6)
        ...
        lbl_pack.pack(anchor="w", padx=15, pady=8)
        btn_br_ios = ctk.CTkButton(card_pack, text="Select Workpath Folder", fg_color="#37474F", hover_color="#455A64", command=self.ios_browse_dest)
        btn_br_ios.pack(side="left", padx=15, pady=(0, 15))
        self.lbl_ios_dest = ctk.CTkLabel(card_pack, text="No local processing path specified.", text_color="#A0A0A5", width=340, anchor="w")
        self.lbl_ios_dest.pack(side="left", padx=5, pady=(0, 15))
        self.btn_build_ipa = ctk.CTkButton(card_pack, text="Build Signed .ipa", fg_color="#00C853", hover_color="#00E676", command=self.start_ipa_build)
        self.btn_build_ipa.pack(side="right", padx=15, pady=(0, 15))
    ...
    # -------------------------------------------------------------------------
    # OPERATIONAL BACKEND DEPENDENCY PARSING MECHANISMS
    # -------------------------------------------------------------------------
    def verify_and_download_dependencies(self):
        """Pre-instantiates folder environment chains and updates status monitors."""
        # 1. Audit and auto-repair pip extension components
        for pkg in self.pip_packages:
            if importlib.util.find_spec(pkg) is None:
                self.log(f"[*] Target Pip Extension module absent. Executing installation stream: {pkg}")
                try:
                    subprocess.check_call([sys.executable, "-m", "pip", "install", pkg], creationflags=subprocess.CREATE_NO_WINDOW)
                    self.log(f"[+] Module library successfully injected into system: {pkg}")
                except Exception as ex:
                    self.log(f"[-] Execution engine failed to run pip installer constraints: {str(ex)}")

        tools_dir = os.path.normpath("tools")
        os.makedirs(tools_dir, exist_ok=True)
        
        if not os.path.exists(os.path.join(tools_dir, "adb.exe")):
            self.log("[!] ENV NOTICE: 'tools/adb.exe' missing. Pre-populate it manually for ADB tasks.")
        ...
        dependencies = {
            ...
            "zipalign.exe": "https://github.com/Aki-S/android-sdk-zipalign-apksigner/raw/master/zipalign.exe",
        ...
        for name, url in dependencies.items():
            ...
            if not os.path.exists(target_path):
                ...
                try:
                    ...
                    self.log(f"[+] Operational package loaded: {name}")
                except Exception as ex:
                    self.log(f"[-] Dependency Extraction Failure on asset '{name}': {str(ex)}")
                    
        self.log("[+] Environmental scanning tasks complete. NK-CyberSuite Engine online and verified.")
        self.check_local_tools_inventory()

    # -------------------------------------------------------------------------
    # OPERATIONAL CORE BUSINESS LOGIC (ADB Pulls, Merging, Rebuilding, SSH)
    # -------------------------------------------------------------------------
    ...
    def adb_fetch_packages_worker(self):
        try:
            ...
            self.log(f"[*] Traced {len(raw_packages)} operational packages. Processing labels...")
            for pkg in sorted(raw_packages):
                info_cmd = ["adb", "shell", f"dumpsys package {pkg} | grep -i 'label=' | head -n 1"]
                info_proc = subprocess.run(info_cmd, capture_output=True, text=True, shell=True, creationflags=subprocess.CREATE_NO_WINDOW)
    ...
    def adb_apk_pull_worker(self, package_id, app_dir):
        try:
            ...
            path_proc = subprocess.run(["adb", "shell", "pm", "path", package_id], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            ...
            self.log(f"[+] Located {len(remote_paths)} binary component file paths on target device.")
            base_local_path = ""
            ...
            for index, remote_path in enumerate(remote_paths):
                ...
                self.log(f"[*] [{index + 1}/{len(remote_paths)}] Extracting component path: {remote_filename}")
                pull_proc = subprocess.Popen(["adb", "pull", remote_path, temp_local_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
                ...
                if os.path.exists(temp_local_path):
                    ...
                    self.log(f"  -> File Staged: {local_filename}")
                    if local_filename in (f"{package_id}.apk", "base.apk"):
                        base_local_path = final_local_path
                else:
                    self.log(f"  [-] Error transferring stream segment: {remote_filename}")

            self.log(f"\n[+] Processing Finished! All pulled binary components saved into:\n -> {app_dir}")
            if base_local_path and os.path.exists(base_local_path):
                ...
                self.log("[!] System Alert: Split app bundle parameters matched. Proceed manually to Step 0b to merge.")
    ...
    def start_apk_merger(self):
        if not self.selected_merge_dir:
            return
    ...
    def apk_merger_worker(self):
        try:
            ...
            merge_proc = subprocess.Popen(merge_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            for line in iter(merge_proc.stdout.readline, ''):
                self.log(f"  [APKEditor] {line.strip()}")
            merge_proc.wait()
            ...
            if os.path.exists(final_output_path):
                ...
                self.lbl_apk.configure(text=os.path.basename(self.target_apk), text_color="#00E676")
                self.btn_decompile.configure(state="normal")
                self.log(f"\n[+] Assembly Forged! Unified standalone structure ready:\n -> {final_output_path}")
                messagebox.showinfo("Merge Complete", f"Compiled cleanly into unified standalone structure!\n{final_output_path}")
            else:
                self.log("[-] Merger finished processing but failed to output a compiled standalone package.")
        except Exception as e:
    ...
    def browse_folder(self):
        ...
        if folderpath:
            if not os.path.exists(os.path.join(folderpath, "apktool.yml")):
                messagebox.showwarning("Warning", "Missing signature parameter: apktool.yml")
            self.rebuild_dir = folderpath
            self.lbl_dir.configure(text=os.path.basename(self.rebuild_dir), text_color="#FFFFFF")
    ...
    def execute_sub_process(self, command_list, terminal_success_msg):
        try:
            process = subprocess.Popen(command_list, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            for line in iter(process.stdout.readline, ''): self.log(line.strip())
            process.stdout.close(); process.wait()
            self.log(f"\n--- {terminal_success_msg if process.returncode == 0 else 'ENGINE ERROR EXECUTION TERMINATED'} ---")
        except Exception as ex: self.log(f"[-] Unexpected core failure mapping tasks: {str(ex)}")
    ...
    def standalone_sign_pipeline_worker(self):
        ...
        self.log("\n[Step 1/2] Running alignment processes...")
        subprocess.run(["tools/zipalign.exe", "-p", "-f", "4", self.unsigned_apk_path, aligned_apk], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=subprocess.CREATE_NO_WINDOW)
        ...
        self.log("[Step 2/2] Cryptographically stamping target package via Apksigner...")
        cmd_sign = ["java", "-jar", "tools/apksigner.jar", "sign", "--ks", "tools/debug.keystore", "--ks-pass", "pass:android", "--out", final_signed_apk, aligned_apk]
        proc = subprocess.run(cmd_sign, capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
    ...
    def rebuild_pipeline_worker(self):
        ...
        self.log("\n[Step 1/3] Compiling source components folder tree via Apktool...")
        subprocess.run(["java", "-jar", "tools/apktool.jar", "b", self.rebuild_dir, "-o", unsigned_apk], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=subprocess.CREATE_NO_WINDOW)
        
        if not os.path.exists(unsigned_apk):
            self.log("[-] Error: Rebuild failed. Review Smali formatting constraints.")
            self.refresh_interface_locks(); return

        self.log("[Step 2/3] Optimization: Running 4-byte boundary zipalign structural processing...")
        subprocess.run(["tools/zipalign.exe", "-p", "-f", "4", unsigned_apk, aligned_apk], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=subprocess.CREATE_NO_WINDOW)

        self.log("[Step 3/3] Cryptographically stamping application framework targets via Apksigner...")
        cmd_sign = ["java", "-jar", "tools/apksigner.jar", "sign", "--ks", "tools/debug.keystore", "--ks-pass", "pass:android", aligned_apk]
        proc = subprocess.run(cmd_sign, capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
        
        if proc.returncode == 0:
            self.log(f"\n--- SUCCESS! Production Modded Signed Target Asset Ready:\n -> {aligned_apk} ---")
    ...
    # -------------------------------------------------------------------------
    # iOS TRANSPORT LINK INTEGRATION
    # -------------------------------------------------------------------------
    ...
    def ios_fetch_apps_worker(self):
        ...
        try:
            ...
            self.log("[+] Transport link established. Parsing mobile application spaces...")
            cmd = (
            ...
            if app_names:
                ...
                self.log(f"[+] Synced {len(app_names)} live remote iOS app configurations.")
            else:
                self.log("[-] Scan yielded empty results. Check device jailbreak configuration layers.")
        except Exception as e:
    ...
    def ios_ipa_build_worker(self, app_name):
        ...
        try:
            ...
            self.log(f"\n[*] Transferring target assets directory over secure SCP pipeline:\n -> {remote_app_path}")
            with SCPClient(ssh.get_transport()) as scp: scp.get(remote_app_path, local_path=payload_dir, recursive=True)

            self.log("[*] Wrapping payload hierarchy structures inside compliance .ipa container...")
        ...
        except Exception as e: self.log(f"[-] Packaging workflow runtime fault: {str(e)}")
